...
interface DetailDrawerContextType {
  openDetail: (type: "request" | "query" | "exception", id: string) => void;
  closeDetail: () => void;
  isOpen: boolean;
  detailType: "request" | "query" | "exception" | null;
...
export function DetailDrawerProvider({ children }: { children: ReactNode }) {
  ...
  const [detailType, setDetailType] = useState<
    "request" | "query" | "exception" | null
  >(null);
  const [detailId, setDetailId] = useState<string | null>(null);

  const openDetail = (type: "request" | "query" | "exception", id: string) => {
    setDetailType(type);
    setDetailId(id);
    setIsOpen(true);
  };
