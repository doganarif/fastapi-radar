...
def create_api_router(get_session_context) -> APIRouter:
    ...
    async def get_requests(
        ...
        requests = query.order_by(desc(CapturedRequest.created_at)).offset(
    ...
    async def get_request_detail(
        ...
        request = session.query(CapturedRequest).filter(
    ...
    async def get_queries(
        ...
        queries = query.order_by(desc(CapturedQuery.created_at)).offset(
            offset).limit(limit).all()
    ...
    async def get_exceptions(
        ...
        if exception_type:
            query = query.filter(
                CapturedException.exception_type == exception_type)

        exceptions = query.order_by(desc(CapturedException.created_at)).offset(
            offset).limit(limit).all()
    ...
    async def get_stats(
        ...
        requests = session.query(CapturedRequest).filter(
            CapturedRequest.created_at >= since
        ).all()

        queries = session.query(CapturedQuery).filter(
            CapturedQuery.created_at >= since
        ).all()

        exceptions = session.query(CapturedException).filter(
            CapturedException.created_at >= since
        ).all()
        ...
        if requests:
            valid_times = [
                r.duration_ms for r in requests if r.duration_ms is not None]
            if valid_times:
        ...
        if queries:
            valid_times = [
                q.duration_ms for q in queries if q.duration_ms is not None]
            if valid_times:
                avg_query_time = sum(valid_times) / len(valid_times)
            slow_queries = len(
                [q for q in queries if q.duration_ms and q.duration_ms >= slow_threshold])
